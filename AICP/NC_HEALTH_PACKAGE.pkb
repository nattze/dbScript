CREATE OR REPLACE PACKAGE BODY AICP."NC_HEALTH_PACKAGE"  IS  
...
END ; --generate_email  
 ...
 BEGIN  
 FOR X in (  
 select decode(user_id ,null ,email,core_ldap.GET_EMAIL_FUNC(user_id)) ldap_mail ,'' ins_name  
 from nc_med_email a  
...
-- and sub_module = (select UPPER(substr(instance_name,1,8)) instance_name from v$instance)  
 ...
 BEGIN  
 --NC_HEALTH_PACKAGE.WRITE_LOG('EMAIL' ,'DB Package mail Bancas' ,'Start: '||v_body ,v_logrst);  
        FOR X in (  
            select decode(user_id ,null ,email,core_ldap.GET_EMAIL_FUNC(user_id)) ldap_mail ,'' ins_name  
            from nc_med_email a  
            where module = 'BANCAS'   
...
--            )VALUES  
            (      
                select sts_key , clm_no ,'' ost_clm_no ,prem_code bene_code ,NC_HEALTH_PACKAGE.GET_PREMCODE_DESCR(prem_code ,'T') descr   
...
--STS_KEY ,B.PREMCODE, B.REQUEST_AMT, B.REMAIN_AMT  
  ...
  FUNCTION GET_FINANCE_STATUS(v_POLICY IN VARCHAR2,  
        ...
        c1   p_actr_package.v_ref_cursor;    
        v_POLNO     VARCHAR2(20);  
        v_POLRUN    NUMBER(20);  
        v_LOSS_DATE DATE;  
  ...
  BEGIN  
       ...
       /*  START count period */   
       p_actr_package.get_pcm_data_cursor(v_POLNO,v_POLRUN,C1);  
        LOOP  
    ...
    BEGIN  
       -- r_chk_pol := NC_HEALTH_PACKAGE.CHECK_POLICY_COVER(null ,vLOSS_DATE ,'X' ,null  ,vIDNO  ,rst_chk_pol);     
        ...
        IF vFlag in ('Y','M') THEN  
            ...
            HEALTHUTIL.get_policy_by_id(vIDNO ,v_lossdate ,c1);  
  ...
  FUNCTION GET_RI_RES(P_POL_NO IN VARCHAR2,  
    ...
    BEGIN  
        FOR X IN (  
            ...
            P_ACTR_PACKAGE.GET_PAYMENT_PAID_INFO(x.pay_no,  
        ...
        /*  
        p_actr_package.get_payment_paid_info(vPayNo,  
                                                   o_vou_date ,  
                                                   o_paid_date,  
                                                   o_amount ,  
                                                   o_pay_method ,  
